...
describe("bash with default options", () => {
    ...
    it("returns exit code 0 for successful commands", async () => {
        const { result } = await handleBashTool({ command: "true" }, {});
        expect(result?.exitCode).toBe(0);
    });

    it("returns non-zero exit code for failed commands", async () => {
        const { result } = await handleBashTool({ command: "false" }, {});
        expect(result?.exitCode).toBe(1);
    });

    it("captures stderr output", async () => {
        const { result } = await handleBashTool({ command: "echo 'error message' >&2" }, {});
        expect(result?.stderr.trim()).toBe("error message");
        expect(result?.stdout.trim()).toBe("");
    });

    it("supports piping between commands", async () => {
        const { result } = await handleBashTool({ command: "echo 'hello world' | tr 'a-z' 'A-Z'" }, {});
        expect(result?.stdout.trim()).toBe("HELLO WORLD");
    });

    it("supports command chaining with &&", async () => {
        const { result } = await handleBashTool({ command: "echo 'first' && echo 'second'" }, {});
        expect(result?.stdout).toContain("first");
        expect(result?.stdout).toContain("second");
    });

    it("handles multi-line output", async () => {
        const { result } = await handleBashTool({ command: "printf 'line1\\nline2\\nline3'" }, {});
    ...
    it("handles commands with arguments and flags", async () => {
        const { result } = await handleBashTool({ command: "echo -n 'no newline'" }, {});
        expect(result?.stdout).toBe("no newline");
    });

    it("supports command substitution", async () => {
        const { result } = await handleBashTool({ command: "echo \"count: $(echo 'a b c' | wc -w | tr -d ' ')\"" }, {});
        expect(result?.stdout.trim()).toBe("count: 3");
    });

    it("returns content string with formatted output", async () => {
        const { content } = await handleBashTool({ command: "echo 'test'" }, {});
