...
- **Optional peer dependencies required for build**: `@google/genai`, `@langchain/core`, `ioredis`, `@aws-sdk/client-s3`, and `@aws-sdk/lib-storage` are peer deps. The DTS build (`npm run build`) fails without `@google/genai` installed. Install all five with `npm install --no-save @google/genai @langchain/core ioredis @aws-sdk/client-s3 @aws-sdk/lib-storage` before building.
